...
BACKUP="$ROOT/backups/phase18_step5_$(date +%Y%m%d_%H%M%S)"

cd "$ROOT"
mkdir -p "$AGENTS" "$CTL" "$MEMORY" "$LOGS" "$RUN" "$BOOT" "$BACKUP"

echo "============================================================"
echo " Phase 18 Step 5 - Watchdog & Auto-Recovery"
echo "============================================================"

for file in \
  "$AGENTS/companyos_watchdog.py" \
  "$CTL/watchdogctl" \
  "$MEMORY/watchdog_config.json" \
  "$MEMORY/watchdog_state.json" \
  "$MEMORY/watchdog_health.json" \
  "$MEMORY/watchdog_audit.json" \
  "$BOOT/start-companyos"
do
  if [ -f "$file" ]; then
    cp -a "$file" "$BACKUP/"
  fi
...
cat > "$MEMORY/watchdog_config.json" <<'JSON'
{
  "enabled": true,
  "check_interval_seconds": 120,
  "automatic_scheduler_restart": true,
  "automatic_integrity_check_after_restart": true,
  "automatic_local_backup_before_recovery": true,
  "maximum_restarts_per_hour": 3,
...
cat > "$AGENTS/companyos_watchdog.py" <<'PY'
#!/usr/bin/env python3
...
chmod +x "$AGENTS/companyos_watchdog.py"
...
root = Path.home() / "companyos"
agent = root / "agents" / "companyos_watchdog.py"
...
cat > "$BOOT/start-companyos" <<'SH'
#!/data/data/com.termux/files/usr/bin/bash

sleep 20
cd "$HOME/companyos" || exit 1

python companyos/operationsctl start \
  >> logs/termux_boot.log 2>&1

python companyos/watchdogctl start \
  >> logs/termux_boot.log 2>&1
SH

chmod +x "$BOOT/start-companyos"
...
echo "[2/6] Checking scheduler..."
python "$CTL/operationsctl" status

echo "[3/6] Running watchdog check..."
python "$CTL/watchdogctl" check

echo "[4/6] Starting watchdog..."
python "$CTL/watchdogctl" start

echo "[5/6] Checking watchdog status..."
...
root = Path.home() / "companyos"
boot = Path.home() / ".termux" / "boot" / "start-companyos"
errors = []

required = [
    root / "agents" / "companyos_watchdog.py",
    root / "companyos" / "watchdogctl",
    root / "ceo_memory" / "watchdog_config.json",
    root / "ceo_memory" / "watchdog_health.json",
    root / "run" / "companyos_watchdog.pid",
    boot,
]
...
try:
    ...
    pid = int(required[4].read_text(encoding="utf-8").strip())
    os.kill(pid, 0)

    health = json.loads(required[3].read_text(encoding="utf-8"))
    if health.get("healthy") is not True:
        errors.append("Watchdog health check is not healthy")

except Exception as exc:
    errors.append(f"Runtime verification failed: {exc}")

print("--------------------------------------------")
print("Phase 18 Step 5 verification")
...
echo
echo "============================================================"
echo " PHASE 18 STEP 5 INSTALLED"
echo " WATCHDOG AND AUTO-RECOVERY RUNNING"
echo " Errors: 0"
echo " Warnings: 0"
echo "============================================================"
echo
echo "Commands:"
echo "  python companyos/watchdogctl status"
echo "  python companyos/watchdogctl check"
echo "  python companyos/watchdogctl recover"
echo "  python companyos/watchdogctl start"
echo "  python companyos/watchdogctl stop"
echo "  python companyos/watchdogctl restart"
echo
echo "Logs:"
echo "  tail -f logs/companyos_watchdog.log"
...
echo "  python companyos/watchdogctl stop"
echo
echo "Phone reboot startup:"
echo "  Install and enable the Termux:Boot app."
