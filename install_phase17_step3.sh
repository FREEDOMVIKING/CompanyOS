...
BACKUP="$ROOT/backups/phase17_step3_$(date +%Y%m%d_%H%M%S)"

cd "$ROOT"
mkdir -p "$AGENTS" "$CTL" "$MEMORY" "$BACKUP"

echo "============================================================"
echo " Phase 17 Step 3 - Executive Priority Engine"
echo "============================================================"

for file in \
  "$AGENTS/executive_priority_engine.py" \
  "$CTL/priorityctl" \
  "$MEMORY/executive_priority_config.json" \
  "$MEMORY/executive_priority_rankings.json" \
  "$MEMORY/executive_priority_briefing.json" \
  "$MEMORY/executive_priority_health.json" \
  "$MEMORY/executive_priority_audit.json"
do
  if [ -f "$file" ]; then
    cp -a "$file" "$BACKUP/"
  fi
...
cat > "$MEMORY/executive_priority_config.json" <<'JSON'
{
  "enabled": true,
  "automatic_internal_ranking": true,
  "automatic_internal_briefing": true,
...
cat > "$AGENTS/executive_priority_engine.py" <<'PY'
#!/usr/bin/env python3
...
chmod +x "$AGENTS/executive_priority_engine.py"

cat > "$CTL/priorityctl" <<'PY'
#!/usr/bin/env python3
import subprocess
import sys
from pathlib import Path
...
root = Path.home() / "companyos"
agent = root / "agents" / "executive_priority_engine.py"
...
echo "[2/5] Ranking executive priorities..."
python "$CTL/priorityctl" rank

echo "[3/5] Checking priority outputs..."
python "$CTL/priorityctl" show
python "$CTL/priorityctl" briefing
python "$CTL/priorityctl" status
...
required = [
    root / "agents" / "executive_priority_engine.py",
    root / "companyos" / "priorityctl",
    root / "ceo_memory" / "executive_priority_config.json",
    root / "ceo_memory" / "executive_priority_rankings.json",
    root / "ceo_memory" / "executive_priority_briefing.json",
    root / "ceo_memory" / "executive_priority_health.json",
]
...
try:
    ...
    priorities = rankings.get("priorities", [])

    if not priorities:
        errors.append("No priorities were ranked")

    for index, item in enumerate(priorities, start=1):
        if item.get("rank") != index:
            errors.append("Priority rank order is invalid")
            break
    ...
    errors.append(f"Priority data error: {exc}")

print("--------------------------------------------")
print("Phase 17 Step 3 verification")
...
echo
echo "============================================================"
echo " PHASE 17 STEP 3 INSTALLED"
echo " Errors: 0"
echo " Warnings: 0"
echo "============================================================"
echo
echo "Commands:"
echo "  python companyos/priorityctl rank"
echo "  python companyos/priorityctl show"
echo "  python companyos/priorityctl briefing"
echo "  python companyos/priorityctl status"
