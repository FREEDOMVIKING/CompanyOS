...
# V65.79a verifies this structurally with AST instead of depending on one exact
# source-code spelling such as "guard.execute" vs "self._v35_guard.execute".
...
try:
    _tree = _ast.parse(dtext)
except Exception as exc:
    raise SystemExit(f"V65_79A_ABORT=dispatcher_ast_parse_failed:{type(exc).__name__}:{exc}")

_has_result_assignment = False
_has_execute_call = False
_has_complete_with_result = False

for _node in _ast.walk(_tree):
    if isinstance(_node, (_ast.Assign, _ast.AnnAssign)):
        _targets = _node.targets if isinstance(_node, _ast.Assign) else [_node.target]
        _names = set()
        for _t in _targets:
            for _sub in _ast.walk(_t):
                if isinstance(_sub, _ast.Name):
                    _names.add(_sub.id)
        if "result" in _names:
            _has_result_assignment = True

    if isinstance(_node, _ast.Call):
        _fn = _node.func
        if isinstance(_fn, _ast.Attribute) and _fn.attr == "execute":
            _has_execute_call = True

        if isinstance(_fn, _ast.Attribute) and _fn.attr == "complete":
            if any(isinstance(_a, _ast.Name) and _a.id == "result" for _a in _node.args):
                _has_complete_with_result = True

print("CURRENT_RESULT_FLOW_AST=", {
    "result_assignment": _has_result_assignment,
    "execute_call": _has_execute_call,
    "complete_with_result": _has_complete_with_result,
})

if not (_has_result_assignment and _has_execute_call and _has_complete_with_result):
    raise SystemExit("V65_79A_ABORT=current_dispatcher_result_flow_not_verified")

print("CURRENT_DISPATCHER_RESULT_FLOW=VERIFIED")

if not DB.exists():
    raise SystemExit("V65_79A_ABORT=execution_kernel_db_missing")
...
if unreadable:
    raise SystemExit("V65_79A_ABORT=unreadable_json")
if integrity_before != "ok":
    raise SystemExit("V65_79A_ABORT=db_integrity_failed")
...
if dup_groups and len(duplicate_repairs) != sum(len(v)-1 for v in dup_groups.values()):
    raise SystemExit("V65_79A_ABORT=not_all_duplicate_keys_are_unambiguous")
...
if unexpected_other:
    raise SystemExit("V65_79A_ABORT=unexpected_state_mismatch_requires_manual_review")
...
for item in duplicate_repairs:
    ...
    if tombstone in db_by_key or tombstone in json_by_key:
        raise SystemExit("V65_79A_ABORT=tombstone_key_collision")
...
if integrity_mid != "ok":
    raise SystemExit("V65_79A_FAIL=db_integrity_after_duplicate_repair")
...
if integrity_after != "ok":
    raise SystemExit("V65_79A_FAIL=db_integrity_after")
if unreadable_after:
    raise SystemExit("V65_79A_FAIL=unreadable_json_after")
if dup_after:
    raise SystemExit("V65_79A_FAIL=json_duplicate_keys_remain")
if conflicts_after:
    raise SystemExit("V65_79A_FAIL=idempotency_owner_conflicts_remain")
if state_mismatches_after:
    raise SystemExit("V65_79A_FAIL=state_projection_mismatches_remain")
...
rp = REPORT_DIR / f"v65_79a_targeted_projection_reconciliation_{STAMP}.json"
rp.write_text(json.dumps(report, indent=2, sort_keys=True, default=str) + "\n")

print("REPORT=", rp)
print("ROLLBACK_AVAILABLE=", SNAP_DIR)
print("V65_79A_DUPLICATE_RECONCILIATION=PASS")
print("V65_79A_HISTORICAL_RESULT_RETRY_RESET=PASS")
print("V65_79A_PROJECTION_ALIGNMENT=PASS")
print("V65_79A_COMPLETE")
PY
