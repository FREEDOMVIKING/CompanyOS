...
MODULE="$ROOT/companyos/runtime/parallel_profit_portfolio.py"
PIDFILE="$RUNTIME/parallel_profit_portfolio.pid"
LOGFILE="$RUNTIME/parallel_profit_portfolio.log"
STATEFILE="$ROOT/.companyos_runtime/parallel_profit_portfolio_state.json"
INTERVAL_SECONDS="${COMPANYOS_PARALLEL_PROFIT_INTERVAL_SECONDS:-900}"
...
case "$action" in
  install)
    echo "===== COMPANYOS V65.93 INSTALL ====="
    install_module
    echo "V65_93_INSTALL=PASS"
    ;;

  once)
    echo "===== COMPANYOS V65.93 ONE CYCLE ====="
    install_module
    python -m companyos.runtime.parallel_profit_portfolio once
    ;;

  status)
    echo "===== COMPANYOS V65.93 STATUS ====="
    if is_running; then
      echo "PARALLEL_PROFIT_MANAGER_RUNNING=true"
      echo "PID=$(cat "$PIDFILE")"
    else
      echo "PARALLEL_PROFIT_MANAGER_RUNNING=false"
    fi
    echo "INTERVAL_SECONDS=$INTERVAL_SECONDS"
    echo "LOGFILE=$LOGFILE"
    if [ -f "$MODULE" ]; then
      python -m companyos.runtime.parallel_profit_portfolio status || true
    fi
    echo "----- LOG TAIL -----"
  ...
  start)
    echo "===== COMPANYOS V65.93 PARALLEL PROFIT PORTFOLIO ====="
    install_module

    if is_running; then
      echo "PARALLEL_PROFIT_MANAGER_ALREADY_RUNNING=true"
      echo "PID=$(cat "$PIDFILE")"
      exit 0
    fi

    echo "===== INITIAL PARALLEL PORTFOLIO CYCLE ====="
    python -m companyos.runtime.parallel_profit_portfolio once

    nohup python -m companyos.runtime.parallel_profit_portfolio loop \
    ...
    if ! kill -0 "$pid" 2>/dev/null; then
      echo "V65_93_ABORT=manager_failed_to_start"
      rm -f "$PIDFILE"
      exit 1
    fi
    ...
    echo "LOGFILE=$LOGFILE"
    echo "V65_93_MANAGER_START=PASS"
    echo "V65_93_COMPLETE"
    ;;

  *)
    echo "Usage: $0 {start|status|stop|restart|once|install}"
    exit 2
    ;;
esac
