...
set -euo pipefail
cd "$HOME/companyos"
echo "===== COMPANYOS PROFIT -> ACTION CLOSURE V21 ====="
TS="$(date +%Y%m%d_%H%M%S)"; BK=".companyos_backups/$TS"; mkdir -p "$BK"
cp companyos/runtime/service_supervisor.py "$BK/service_supervisor.py"

cat > companyos/runtime/profit_to_action_closure.py <<'PY'
...
needle='        if (Path.home()/"companyos/companyos/runtime/continuous_profit_improvement_loop.py").exists():\n'
block=(
...
if "profit_to_action_closure" not in s:
    if needle not in s: raise SystemExit("supervisor insertion anchor not found")
    p.write_text(s.replace(needle,block+needle,1))
PY

mkdir -p tests/generated
cat > tests/generated/test_profit_to_action_closure_v21.py <<'PY'
...
echo "===== COMPILE + TEST ====="
python -m py_compile companyos/runtime/profit_to_action_closure.py companyos/runtime/service_supervisor.py
python -m pytest -q tests/generated/test_profit_to_action_closure_v21.py

echo "===== ONE GUARDED CYCLE ====="
python -m companyos.runtime.profit_to_action_closure once || true

echo "===== START WITHOUT RESTARTING HEALTHY SUPERVISOR ====="
if ! pgrep -f 'companyos.runtime.profit_to_action_closure' >/dev/null 2>&1; then
  nohup python -u -m companyos.runtime.profit_to_action_closure run > .companyos_runtime/profit_to_action_closure_console.log 2>&1 &
  echo $! > .companyos_runtime/profit_to_action_closure.pid
fi
sleep 2; pgrep -af 'companyos.runtime.profit_to_action_closure' || true

echo "===== PRESERVE SUPERVISOR ====="
ps -ef | grep 'companyos/runtime/service_supervisor.py' | grep -v grep || true

echo "===== COMMIT/PUSH EXPLICIT FILES ONLY ====="
git add companyos/runtime/profit_to_action_closure.py companyos/runtime/service_supervisor.py tests/generated/test_profit_to_action_closure_v21.py
git commit -m "Add guarded profit-to-action closure loop" || true
git push origin "$(git branch --show-current)"

echo
echo "COMPANYOS_PROFIT_TO_ACTION_CLOSURE_V21=PASS"
