...
echo "===== COMPANYOS V66.31A MULTI-PROVIDER PROSPECT FALLBACK ====="
echo "GOAL=REPAIR_V66_31_REGEX_AND_KEEP_PROSPECT_DISCOVERY_RUNNING_WHEN_OPENAI_WEB_SEARCH_IS_RATE_LIMITED"
...
[ -f "$MOD" ] || { echo "V66_31A_ABORT=missing:$MOD"; exit 1; }

stamp="$(date +%Y%m%d_%H%M%S)"
cp "$MOD" "${MOD}.v66_31a_backup_${stamp}"
echo "BACKUP=${MOD}.v66_31a_backup_${stamp}"
...
def replace_fn(src,name,new_text):
    sp=fn_span(src,name)
    if not sp:
        raise SystemExit(f"V66_31A_ABORT=function_not_found:{name}")
...
if "def openai_web_search(" not in s:
    ...
    if not sp:
        raise SystemExit("V66_31A_ABORT=web_search_missing")
    lines=s.splitlines()
    first=lines[sp[0]-1]
...
if "def provider_fallback_search(" not in s:
    ...
    if not sp:
        raise SystemExit("V66_31A_ABORT=normalize_url_missing")
    lines=s.splitlines()
...
ast.parse(s)
p.write_text(s)
print("V66_31A_PROVIDER_ROUTER_PATCH=PASS")
PY
...
echo "V66_31A_OPENAI_429_FALLBACK=PASS"
echo "V66_31A_BRAVE_IF_CONFIGURED=PASS"
echo "V66_31A_KEYLESS_DDG_FALLBACK=PASS"
echo "V66_31A_ROLE_BASED_BUSINESS_EMAIL_ONLY=PASS"
echo "V66_31A_EXACT_SOURCE_VERIFICATION_PRESERVED=PASS"
echo "V66_31A_V66_27_HANDOFF=PASS"
echo "V66_31A_NO_NEW_DAEMON=PASS"
echo "V66_31A_NO_FINANCIAL_ACTIONS=PASS"
echo "V66_31A_AUTHORITY_SWITCHES_UNCHANGED=PASS"
echo "V66_31A_COMPLETE"
