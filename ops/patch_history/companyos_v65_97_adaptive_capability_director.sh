...
PID="$GRT/adaptive_capability_director.pid"
LOG="$GRT/adaptive_capability_director.log"
...
def requirements(inv,rows):
 ...
 miss=[r for r in ded.values() if not present(r["id"],inv)]
...
def build(req):
 try:
  ...
  gap={"id":req["id"],"title":req["id"].replace("_"," ").title(),
  ...
  try:r=ce.cycle()
  finally:ce.derive_gap=old
  status=str(r.get("status") or "")
  ok=status in {"capability_promoted_and_used","existing_capability_used"}
  if ok:
   fb=load(FEEDBACK,{"capabilities":{}});caps=fb.get("capabilities")
   if not isinstance(caps,dict):caps={}
   rec=caps.get(req["id"]) if isinstance(caps.get(req["id"]),dict) else {}
   rec.update({"status":"active","utility_score":float(rec.get("utility_score",50) or 50),
               "source":"adaptive_capability_director_v65_97","kind":req["kind"],
               "activated_at_unix":time.time()})
   caps[req["id"]]=rec;fb["capabilities"]=caps;fb["updated_at_unix"]=time.time();save(FEEDBACK,fb)
  return {"ok":ok,"status":status}
 except Exception as e:return {"ok":False,"status":"exception","error":repr(e)}
...
case "${1:-start}" in
 start)
  echo "===== COMPANYOS V65.97 ADAPTIVE CAPABILITY DIRECTOR ====="
  install_module
  if running; then echo "CAPABILITY_DIRECTOR_ALREADY_RUNNING=true";echo "PID=$(cat "$PID")";exit 0;fi
  echo "===== INITIAL SELF-EXPANSION CYCLE ====="
  python -m companyos.runtime.adaptive_capability_director once
  nohup python -m companyos.runtime.adaptive_capability_director loop --interval "$INTERVAL" >>"$LOG" 2>&1 &
  p="$!";echo "$p" >"$PID";sleep 1
  kill -0 "$p" 2>/dev/null || { rm -f "$PID";echo "V65_97_ABORT=director_failed_to_start";exit 1; }
  echo "CAPABILITY_DIRECTOR_RUNNING=true"
  echo "PID=$p"
  echo "INTERVAL_SECONDS=$INTERVAL"
  echo "LOGFILE=$LOG"
  echo "V65_97_MANAGER_START=PASS"
  echo "V65_97_COMPLETE"
  ;;
 once) install_module;python -m companyos.runtime.adaptive_capability_director once ;;
 status)
  echo "===== COMPANYOS V65.97 STATUS ====="
  if running;then echo "CAPABILITY_DIRECTOR_RUNNING=true";echo "PID=$(cat "$PID")";else echo "CAPABILITY_DIRECTOR_RUNNING=false";fi
  [ -f "$MOD" ] && python -m companyos.runtime.adaptive_capability_director status || true
  echo "----- LOG TAIL -----";tail -n 100 "$LOG" 2>/dev/null || true
  ;;
 inventory) install_module;python -m companyos.runtime.adaptive_capability_director inventory ;;
 stop)
  if running;then p="$(cat "$PID")";kill "$p" 2>/dev/null || true;sleep 1;kill -9 "$p" 2>/dev/null || true;fi
  rm -f "$PID";echo "V65_97_STOP=PASS"
  ;;
 restart) "$0" stop || true;exec "$0" start ;;
 *) echo "Usage: $0 {start|once|status|inventory|stop|restart}";exit 2 ;;
esac
