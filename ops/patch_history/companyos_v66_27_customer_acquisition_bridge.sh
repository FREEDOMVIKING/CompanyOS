...
MOD="$ROOT/companyos/runtime/customer_acquisition_bridge.py"
CTL="$ROOT/scripts/companyos_acquisitionctl"
...
echo "===== COMPANYOS V66.27 CUSTOMER ACQUISITION BRIDGE ====="
echo "GOAL=TURN_LAUNCHED_VENTURES_INTO_MEASURABLE_REAL_CUSTOMER_ACQUISITION_ATTEMPTS"
echo "NOTE=ONLY_EXPLICIT_PUBLIC_BUSINESS_CONTACTS_ARE_ELIGIBLE"
echo "NOTE=ONE_OUTREACH_ACTION_PER_RUN_AND_DAILY_CAP"
...
for f in \
  ...
  "$ROOT/companyos/runtime/live_drl_strategy_governor.py"
do
  [ -f "$f" ] || { echo "V66_27_ABORT=missing:$f"; exit 1; }
done
...
case "${1:-once}" in
  once)
    python - <<'PY'
import json
from companyos.runtime.customer_acquisition_bridge import run_once
print(json.dumps(run_once(),indent=2,sort_keys=True,default=str))
PY
    ;;
  status)
    cat "$HOME/.companyos_runtime/customer_acquisition_bridge_latest.json" 2>/dev/null || echo '{"status":"no_run_yet"}'
    ;;
  *)
...
cat > "$ROOT/tests/test_v66_27_customer_acquisition_bridge.py" <<'PY'
...
echo "V66_27_MODULE_COMPILE=PASS"

echo "===== TEST ====="
python -m pytest -q tests/test_v66_27_customer_acquisition_bridge.py
echo "V66_27_TESTS=PASS"
...
echo "===== FIRST CUSTOMER ACQUISITION PASS ====="
"$CTL" once

echo "===== ALLOW EXTERNAL ROUTER TO PROCESS ONE ACTION ====="
sleep 12

echo "===== RECONCILE OUTREACH RESULT ====="
"$CTL" once
...
echo "V66_27_LAUNCH_TO_ACQUISITION_BRIDGE=PASS"
echo "V66_27_PUBLIC_BUSINESS_CONTACT_GATE=PASS"
echo "V66_27_PROVENANCE_REQUIRED=PASS"
echo "V66_27_ONE_ACTION_PER_RUN=PASS"
echo "V66_27_DAILY_CAP=PASS"
echo "V66_27_REAL_SMTP_EVIDENCE_REQUIRED=PASS"
echo "V66_27_NO_FINANCIAL_ACTIONS=PASS"
echo "V66_27_AUTHORITY_SWITCHES_UNCHANGED=PASS"
echo "V66_27_COMPLETE"
