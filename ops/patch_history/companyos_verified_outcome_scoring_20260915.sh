...
echo "===== VERIFIED OUTCOME SCORING ====="
echo "Promotion now requires downstream evidence, not repeated file hits."
echo "No finance transfer. No DNS mutation. No supervisor restart."

mkdir -p companyos/runtime scripts tests/generated .companyos_runtime/verified_outcomes

cat > companyos/runtime/verified_outcome_scoring.py <<'PY'
...
cat > scripts/companyos_verify_worker_outcomes <<'PY'
#!/data/data/com.termux/files/usr/bin/python
import json,sys
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]; sys.path.insert(0,str(ROOT))
from companyos.runtime.verified_outcome_scoring import VerifiedOutcomeScorer
print(json.dumps(VerifiedOutcomeScorer(ROOT).run(),indent=2,sort_keys=True))
PY
chmod +x scripts/companyos_verify_worker_outcomes
...
def test_activity_alone_not_permanent(tmp_path):
    ...
    w={"role":"candidate_qualification","jobs":3,"successes":3,"failures":0,"status":"permanent"}
    r=s.evaluate_worker(w,[])
    assert r["status"]=="probation"
    assert r["verified_outcome_score"]==0

def test_real_outcomes_can_promote(tmp_path):
    s=VerifiedOutcomeScorer(tmp_path)
...
echo "===== COMPILE ====="
python -m py_compile companyos/runtime/verified_outcome_scoring.py scripts/companyos_verify_worker_outcomes

echo "===== TEST ====="
python -m pytest -q tests/generated/test_verified_outcome_scoring.py

echo "===== LIVE EVALUATION ====="
python scripts/companyos_verify_worker_outcomes | tee .companyos_runtime/verified_outcomes/commissioning.json

echo "===== COMMIT ONLY THIS LAYER ====="
git add companyos/runtime/verified_outcome_scoring.py scripts/companyos_verify_worker_outcomes tests/generated/test_verified_outcome_scoring.py
git commit -m "require verified downstream outcomes for autonomous worker promotion" || true
...
print("COMPANYOS_VERIFIED_OUTCOME_SCORING=PASS")
PY
