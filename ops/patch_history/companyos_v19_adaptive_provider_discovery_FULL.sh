...
set -euo pipefail
R="$HOME/companyos"; B="$R/backups/v19_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$R/companyos/adaptive_provider_discovery_v19" "$R/ceo_memory" "$R/config" "$B"
[ -f "$R/.env" ] || { echo "FAIL: ~/companyos/.env missing"; exit 1; }
cp -a "$R/config/connectors_v18.json" "$B/" 2>/dev/null || true
cat > "$R/companyos/adaptive_provider_discovery_v19/discovery.py" <<'PY'
import json,re
...
PY
cat > "$R/companyos/adaptive_provider_discovery_v19/__init__.py" <<'PY'
from .discovery import discover,build,validate
PY
cat > "$R/companyos_v19ctl" <<'PY'
#!/usr/bin/env python3
import json,sys
from pathlib import Path
sys.path.insert(0,str(Path.home()/"companyos"))
from companyos.adaptive_provider_discovery_v19.discovery import discover,build,validate
c=sys.argv[1] if len(sys.argv)>1 else "status"
f={"status":discover,"build-config":build,"validate":validate}.get(c)
if not f: raise SystemExit("usage: companyos_v19ctl {status|build-config|validate}")
print(json.dumps(f(),indent=2))
PY
chmod +x "$R/companyos_v19ctl"
echo "[1/4] compile"; python -m py_compile "$R/companyos/adaptive_provider_discovery_v19/"*.py "$R/companyos_v19ctl"
echo "[2/4] discover"; python "$R/companyos_v19ctl" status
echo "[3/4] validate"; python "$R/companyos_v19ctl" validate
echo "[4/4] restart V18"; cd "$R"; bash companyosctl restart || bash companyosctl start
bash companyosctl status
echo
echo "V19 ADAPTIVE PROVIDER DISCOVERY INSTALLED"
echo "Backup: $B"
echo "Run: python ~/companyos/companyos_v19ctl validate"
echo "No secret values printed; no wallet/seed scanning; no external actions."
