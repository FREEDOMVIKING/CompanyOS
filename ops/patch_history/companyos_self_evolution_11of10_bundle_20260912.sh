...
BACKUP="$ROOT/.companyos_backups/self_evolution_11of10_$STAMP"
cd "$ROOT"
mkdir -p "$BACKUP" "$ROOT/.companyos_runtime/self_evolution"

echo "============================================================"
echo " CompanyOS SELF-EVOLUTION 11/10 ARCHITECTURE"
echo " Shadow build -> guard -> test -> promote -> monitor -> rollback"
echo "============================================================"

for f in companyos/runtime/service_supervisor.py companyos/runtime/self_evolution_engine.py companyos/runtime/self_evolution_runtime.py scripts/companyos_evolutionctl tests/test_self_evolution_guard.py; do
  if [ -f "$f" ]; then mkdir -p "$BACKUP/$(dirname "$f")"; cp -a "$f" "$BACKUP/$f"; fi
done

cat > companyos/runtime/self_evolution_engine.py <<'PY'
...
cat > companyos/runtime/self_evolution_runtime.py <<'PY'
...
cat > scripts/companyos_evolutionctl <<'PY'
...
main()
PY
chmod +x scripts/companyos_evolutionctl

cat > tests/test_self_evolution_guard.py <<'PY'
import unittest
...
needle='''        if (Path.home()/"companyos/companyos/runtime/profit_opportunity_runtime.py").exists():
...
if "self_evolution_runtime" not in s:
    if needle not in s:raise SystemExit("ERROR: supervisor insertion point not found; refusing unsafe rewrite")
    p.write_text(s.replace(needle,addition,1)); print("SUPERVISOR_PATCH=APPLIED")
else:print("SUPERVISOR_PATCH=ALREADY_PRESENT")
PY

python - <<'PY'
from pathlib import Path
p=Path.home()/".companyos_launch_env"; lines=p.read_text().splitlines() if p.exists() else []
settings={"COMPANYOS_ENABLE_SELF_EVOLUTION":"1","COMPANYOS_SELF_EVOLUTION_AUTO_PROMOTE":"1","COMPANYOS_SELF_EVOLUTION_AUTO_RESTART":"1","COMPANYOS_SELF_EVOLUTION_PUSH":"1","COMPANYOS_SELF_EVOLUTION_INTERVAL_SECONDS":"1800","COMPANYOS_SELF_EVOLUTION_INITIAL_DELAY_SECONDS":"60","COMPANYOS_SELF_EVOLUTION_MAX_DAILY_PROPOSALS":"4","COMPANYOS_SELF_EVOLUTION_MAX_FILES":"4","COMPANYOS_SELF_EVOLUTION_MAX_LINES":"600","COMPANYOS_SELF_EVOLUTION_FULL_TESTS":"1","COMPANYOS_SELF_EVOLUTION_TEST_TIMEOUT":"240","COMPANYOS_SELF_EVOLUTION_GUARD_SECONDS":"900"}
for k,v in settings.items():lines=[x for x in lines if not x.startswith(f"export {k}=")]; lines.append(f"export {k}={v}")
p.write_text("\n".join(lines)+"\n"); p.chmod(0o600); print("SELF_EVOLUTION_ENV=CONFIGURED")
PY

source "$ENV_FILE"
echo "[QUALIFY] syntax"
python -m py_compile companyos/runtime/self_evolution_engine.py companyos/runtime/self_evolution_runtime.py companyos/runtime/service_supervisor.py scripts/companyos_evolutionctl tests/test_self_evolution_guard.py

echo "[QUALIFY] immutable guard"
python -m unittest tests.test_self_evolution_guard

echo "[QUALIFY] diagnosis"
scripts/companyos_evolutionctl diagnose | tee "$ROOT/.companyos_runtime/self_evolution/install_diagnosis_$STAMP.json"

echo "[QUALIFY] secret scan"
for secret_name in OPENAI_API_KEY CLOUDFLARE_API_TOKEN SOLANA_PRIVATE_KEY SMTP_PASSWORD; do
  secret="${!secret_name:-}"
  if [ -n "$secret" ] && git grep -nF "$secret" -- . ':!*.log' ':!.companyos_runtime' 2>/dev/null | head -1 | grep -q .; then
    echo "ERROR: value of $secret_name found in source; refusing commit"; exit 1
  fi
done
echo "SECRET_SCAN=PASS"

git add -- companyos/runtime/self_evolution_engine.py companyos/runtime/self_evolution_runtime.py companyos/runtime/service_supervisor.py scripts/companyos_evolutionctl tests/test_self_evolution_guard.py
if ! git diff --cached --quiet; then git commit -m "Add guarded autonomous self-evolution engine"; fi
BRANCH="$(git branch --show-current)"
if [ -n "$BRANCH" ]; then git push origin "$BRANCH" || echo "WARNING: push failed; local self-evolution install remains valid"; fi

echo "[LAUNCH] restarting supervisor"
if [ -x scripts/companyosctl ]; then scripts/companyosctl restart || true; elif [ -x scripts/companyos_control ]; then scripts/companyos_control restart || true; fi
sleep 5

echo "================ SELF-EVOLUTION STATUS ================"
scripts/companyos_evolutionctl status
...
ss=d.get("services") or {}; print("SUPERVISOR_RUNNING="+str(bool(d.get("running"))).lower()); print("SERVICE_COUNT="+str(len(ss)))
for n,r in ss.items():print(f"{n}: running={bool(r.get('running'))} failures={r.get('consecutive_failures',0)}")
print("SELF_EVOLUTION_SUPERVISED="+str("self_evolution_runtime" in ss).lower())
PY

echo "Backup: $BACKUP"
echo "Control: scripts/companyos_evolutionctl status"
echo "Force one guarded cycle: scripts/companyos_evolutionctl once"
echo "Proposal only: scripts/companyos_evolutionctl proposal"
echo "First autonomous evolution cycle begins after ~60 seconds."
echo "Protected: wallet/finance/credentials/approvals/security/connectors/supervisor/self-evolution guard."
echo "COMPANYOS_SELF_EVOLUTION_11OF10=PASS"
