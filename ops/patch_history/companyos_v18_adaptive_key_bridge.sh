...
cat > "$ROOT/adaptive_keys_v18ctl" <<'PY'
#!/usr/bin/env python3
import json,sys
from pathlib import Path
sys.path.insert(0,str(Path.home()/"companyos"))
from companyos.adaptive_key_bridge_v18.resolver import AdaptiveKeyResolverV18
r=AdaptiveKeyResolverV18(); cmd=sys.argv[1] if len(sys.argv)>1 else "status"
out=r.status() if cmd=="status" else r.build_map() if cmd=="discover" else r.export() if cmd=="export" else None
if out is None: raise SystemExit("usage: adaptive_keys_v18ctl {status|discover|export}")
print(json.dumps(out,indent=2))
PY
chmod +x "$ROOT/adaptive_keys_v18ctl"

cat > "$ROOT/load_adaptive_keys_v18.sh" <<'SH'
#!/data/data/com.termux/files/usr/bin/bash
set -euo pipefail
ROOT="$HOME/companyos"
...
SH
chmod +x "$ROOT/load_adaptive_keys_v18.sh"

python -m py_compile "$PKG"/*.py "$ROOT/adaptive_keys_v18ctl"
python "$ROOT/adaptive_keys_v18ctl" status
echo
echo "Adaptive key bridge installed."
echo "Run:"
echo "  cd ~/companyos"
echo "  . ./load_adaptive_keys_v18.sh"
echo "  bash companyosctl env-check"
echo
echo "Wallet/private/seed credentials are intentionally excluded."
