...
CTL="$ROOT/companyos"
MEM="$ROOT/ceo_memory"
BACKUP="$ROOT/backups/phase20_step9_$(date +%Y%m%d_%H%M%S)"

cd "$ROOT"
mkdir -p "$AGENTS" "$CTL" "$MEM" "$BACKUP"

echo "============================================================"
echo " Phase 20 Step 9 - Adaptive Execution Plan Builder"
echo "============================================================"

for f in \
  "$AGENTS/adaptive_execution_plan.py" \
  "$CTL/executionplan2ctl" \
  "$MEM/execution_plan2_config.json" \
  "$MEM/execution_plan2_state.json" \
  "$MEM/execution_plan2_report.json" \
  "$MEM/execution_plan2_health.json" \
  "$MEM/autonomous_operations_config.json"
do
  [ -f "$f" ] && cp -a "$f" "$BACKUP/"
done

cat > "$MEM/execution_plan2_config.json" <<'JSON'
{
  "enabled": true,
  "automatic_plan_building": true,
  "maximum_plan_actions": 5,
  "preserve_priority_order": true,
  "require_selected_action_eligibility": true,
...
cat > "$AGENTS/adaptive_execution_plan.py" <<'PY'
#!/usr/bin/env python3
...
chmod +x "$AGENTS/adaptive_execution_plan.py"

cat > "$CTL/executionplan2ctl" <<'PY'
#!/usr/bin/env python3
import subprocess
import sys
from pathlib import Path
...
root = Path.home() / "companyos"
agent = root / "agents" / "adaptive_execution_plan.py"
...
chmod +x "$CTL/executionplan2ctl"

echo "[1/6] Compiling..."
python -m py_compile "$AGENTS/adaptive_execution_plan.py" "$CTL/executionplan2ctl"

echo "[2/6] Building adaptive execution plan..."
python "$CTL/executionplan2ctl" build
...
job = {
    "id": "adaptive-execution-plan",
    "enabled": True,
    "interval_seconds": 1800,
    "command": ["python", "companyos/executionplan2ctl", "build"]
}
...
echo "[5/6] Checking execution plan status..."
python "$CTL/executionplan2ctl" status
...
required = [
    root / "agents" / "adaptive_execution_plan.py",
    root / "companyos" / "executionplan2ctl",
    root / "ceo_memory" / "execution_plan2_config.json",
    root / "ceo_memory" / "execution_plan2_state.json",
    root / "ceo_memory" / "execution_plan2_report.json",
    root / "ceo_memory" / "execution_plan2_health.json",
...
try:
    ...
    if "plan" not in report:
        errors.append("Execution plan report missing plan")

    sched = json.loads(required[6].read_text())
    job = next(
        (x for x in sched.get("jobs", []) if x.get("id") == "adaptive-execution-plan"),
        None
    )
    if not job or job.get("enabled") is not True:
        errors.append("Adaptive execution plan scheduler job missing/disabled")

except Exception as exc:
    errors.append(str(exc))

print("--------------------------------------------")
print("Phase 20 Step 9 verification")
...
echo
echo "============================================================"
echo " PHASE 20 STEP 9 INSTALLED"
echo " ADAPTIVE EXECUTION PLAN BUILDER ACTIVE"
echo " Errors: 0"
echo " Warnings: 0"
echo "============================================================"
echo
echo "Commands:"
echo "  python companyos/executionplan2ctl build"
echo "  python companyos/executionplan2ctl status"
