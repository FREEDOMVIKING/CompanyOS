...
EVENT_FILES=[
 ROOT/"logs/autonomous_operations.log",
 ROOT/".companyos_runtime/supervisor_v4/events.jsonl",
...
def classify_log_line(text):
    low=text.lower()
    if "starting job " in low:
        return "autonomous_job_started"
    if "finished job " in low:
        return "autonomous_job_finished"
    if "cycle complete:" in low:
        return "autonomous_cycle_complete"
