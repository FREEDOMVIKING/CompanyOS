...
def empty_cycle_detected(min_idle_cycles=25):
    ...
    if not existing or "last_productive_cycle_count" not in st:
        seed = cycle_count
        if active == 0 and dispatched == 0 and not last_oid and cycle_count >= min_idle_cycles:
            seed = max(0, cycle_count - min_idle_cycles)
        st = {
            "last_cycle_count": cycle_count,
            "last_productive_cycle_count": seed,
            "recoveries": st.get("recoveries", []),
            "initialized_at_unix": time.time(),
        }
...
def recovery_goal():
    ...
    return base + (
        ...
        "- Produce at least one persisted state change: task queue entry, research artifact, ranked opportunity file, validation plan, or orchestration record.\n"
        ...
        "- Keep consequential external actions, financial transactions, spending, credential changes, destructive actions, and irreversible commitments behind existing approval and safety gates."
    )
