...
BACKUP="$BACKUPS/phase18_step2_$(date +%Y%m%d_%H%M%S)"

cd "$ROOT"
mkdir -p "$AGENTS" "$CTL" "$MEMORY" "$BACKUPS" "$BACKUP"

echo "============================================================"
echo " Phase 18 Step 2 - System Integrity & Backup Manager"
echo "============================================================"

for file in \
  "$AGENTS/system_integrity_manager.py" \
  "$CTL/integrityctl" \
  "$MEMORY/system_integrity_config.json" \
  "$MEMORY/system_integrity_report.json" \
  "$MEMORY/system_integrity_health.json" \
  "$MEMORY/system_integrity_audit.json"
do
  if [ -f "$file" ]; then
    cp -a "$file" "$BACKUP/"
  fi
...
cat > "$MEMORY/system_integrity_config.json" <<'JSON'
{
  "enabled": true,
  "automatic_internal_integrity_checks": true,
  "automatic_local_backups": true,
  ...
  "owner_approval_required_for_repairs": true
}
JSON

cat > "$AGENTS/system_integrity_manager.py" <<'PY'
#!/usr/bin/env python3
...
chmod +x "$AGENTS/system_integrity_manager.py"

cat > "$CTL/integrityctl" <<'PY'
#!/usr/bin/env python3
import subprocess
import sys
from pathlib import Path
...
root = Path.home() / "companyos"
agent = root / "agents" / "system_integrity_manager.py"
...
python "$CTL/integrityctl" check

echo "[3/5] Creating local backup..."
python "$CTL/integrityctl" backup
...
required = [
    root / "agents" / "system_integrity_manager.py",
    root / "companyos" / "integrityctl",
    root / "ceo_memory" / "system_integrity_config.json",
    root / "ceo_memory" / "system_integrity_report.json",
    root / "ceo_memory" / "system_integrity_health.json",
]
...
try:
    ...
    if report.get("summary", {}).get("errors") != 0:
        errors.append("Integrity report contains errors")

    snapshots = list((root / "backups").glob("system_snapshot_*"))
    if not snapshots:
        errors.append("No system snapshot was created")

except Exception as exc:
    errors.append(f"Verification data error: {exc}")

print("--------------------------------------------")
print("Phase 18 Step 2 verification")
...
echo
echo "============================================================"
echo " PHASE 18 STEP 2 INSTALLED"
echo " Errors: 0"
echo " Warnings: 0"
echo "============================================================"
echo
echo "Commands:"
echo "  python companyos/integrityctl check"
echo "  python companyos/integrityctl backup"
echo "  python companyos/integrityctl list-backups"
echo "  python companyos/integrityctl status"
