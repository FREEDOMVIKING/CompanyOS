...
def manifest():
    return json.loads((ROOT/"audit/COMPANYOS_V67_5_FINAL_LEGACY_EXTRACTION_MANIFEST.json").read_text())

def tracked():
    cp=subprocess.run(["git","ls-files"],cwd=ROOT,text=True,capture_output=True,check=True)
    return set(cp.stdout.splitlines())

def test_final_batch_is_zero_reference_and_recoverable():
    d=manifest()
    assert d["selected_root_count"] > 0
    assert d["files_removed_from_active_branch"] > 0
    assert d["recovery_branch"].startswith("recovery/v67.5-pre-final-legacy-extraction-")
