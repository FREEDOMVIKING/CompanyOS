...
BACKUP="$ROOT/backups/phase16_step5_$(date +%Y%m%d_%H%M%S)"

cd "$ROOT"
mkdir -p "$AGENTS" "$CTL" "$MEMORY" "$BACKUP"

echo "============================================================"
echo " Phase 16 Step 5 - Business Automation Engine"
echo "============================================================"

for file in \
  "$AGENTS/business_automation_engine.py" \
  "$CTL/automationctl" \
  "$MEMORY/business_automation_config.json" \
  "$MEMORY/business_automation_state.json" \
  "$MEMORY/business_activity_feed.json" \
  "$MEMORY/daily_executive_summary.json" \
  "$MEMORY/business_automation_health.json"
do
  if [ -f "$file" ]; then
    cp -a "$file" "$BACKUP/"
  fi
...
cat > "$MEMORY/business_automation_config.json" <<'JSON'
{
  "enabled": true,
  "automatic_internal_task_creation": true,
  "automatic_quote_to_project_linking": true,
  "automatic_milestone_generation": true,
  "automatic_activity_feed": true,
  "automatic_daily_summary": true,
...
[ -f "$MEMORY/business_automation_state.json" ] || cat > "$MEMORY/business_automation_state.json" <<'JSON'
{
  "schema_version": 1,
  "processed_leads": [],
  "processed_quotes": [],
...
[ -f "$MEMORY/business_activity_feed.json" ] || cat > "$MEMORY/business_activity_feed.json" <<'JSON'
{
  "schema_version": 1,
  "activities": [],
...
cat > "$AGENTS/business_automation_engine.py" <<'PY'
#!/usr/bin/env python3
...
chmod +x "$AGENTS/business_automation_engine.py"

cat > "$CTL/automationctl" <<'PY'
#!/usr/bin/env python3
import subprocess
import sys
from pathlib import Path
...
root = Path.home() / "companyos"
agent = root / "agents" / "business_automation_engine.py"
...
echo "[2/5] Running internal automation cycle..."
python "$CTL/automationctl" run

echo "[3/5] Checking automation outputs..."
python "$CTL/automationctl" show-summary
python "$CTL/automationctl" feed
python "$CTL/automationctl" status
...
required = [
    root / "agents" / "business_automation_engine.py",
    root / "companyos" / "automationctl",
    root / "ceo_memory" / "business_automation_config.json",
    root / "ceo_memory" / "business_automation_state.json",
    root / "ceo_memory" / "business_activity_feed.json",
    root / "ceo_memory" / "daily_executive_summary.json",
]
...
try:
    ...
    if not feed:
        errors.append("Business activity feed is empty")
    ...
    errors.append(f"Automation memory error: {exc}")

print("--------------------------------------------")
print("Phase 16 Step 5 verification")
...
echo
echo "============================================================"
echo " PHASE 16 STEP 5 INSTALLED"
echo " Errors: 0"
echo " Warnings: 0"
echo "============================================================"
echo
echo "Commands:"
echo "  python companyos/automationctl run"
echo "  python companyos/automationctl summary"
echo "  python companyos/automationctl show-summary"
echo "  python companyos/automationctl feed"
echo "  python companyos/automationctl status"
