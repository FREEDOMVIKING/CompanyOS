...
BACKUP="$ROOT/backups/phase16_step1_$(date +%Y%m%d_%H%M%S)"

cd "$ROOT"
mkdir -p "$AGENTS" "$CTL" "$MEMORY" "$BACKUP"

echo "============================================================"
echo " Phase 16 Step 1 - CRM and Lead Management"
echo "============================================================"

for file in \
  "$AGENTS/crm_manager.py" \
  "$CTL/crmctl" \
  "$MEMORY/crm_config.json" \
  "$MEMORY/crm_customers.json" \
  "$MEMORY/crm_leads.json" \
  "$MEMORY/crm_activities.json" \
  "$MEMORY/crm_health.json"
do
  if [ -f "$file" ]; then
    cp -a "$file" "$BACKUP/"
  fi
...
cat > "$MEMORY/crm_config.json" <<'JSON'
{
  "enabled": true,
...
[ -f "$MEMORY/crm_customers.json" ] || cat > "$MEMORY/crm_customers.json" <<'JSON'
{
  "schema_version": 1,
  "customers": [],
  "statistics": {
...
[ -f "$MEMORY/crm_leads.json" ] || cat > "$MEMORY/crm_leads.json" <<'JSON'
{
  "schema_version": 1,
  "leads": [],
  "statistics": {
...
[ -f "$MEMORY/crm_activities.json" ] || cat > "$MEMORY/crm_activities.json" <<'JSON'
{
  "schema_version": 1,
  "activities": [],
  "last_updated_at": null
...
root = Path.home() / "companyos"
agent = root / "agents" / "crm_manager.py"
...
echo "[2/5] Creating safe sample customer and lead..."
python "$CTL/crmctl" add-customer \
  "Sample Contractor" \
  "555-0100" \
  "sample@example.com"

python "$CTL/crmctl" add-lead \
  "Sample Contractor" \
  "Driveway Replacement" \
  "12500" \
  "internal_test"

echo "[3/5] Checking CRM data..."
python "$CTL/crmctl" customers
python "$CTL/crmctl" leads
python "$CTL/crmctl" activities
python "$CTL/crmctl" status
...
required = [
    root / "agents" / "crm_manager.py",
    root / "companyos" / "crmctl",
    root / "ceo_memory" / "crm_config.json",
    root / "ceo_memory" / "crm_customers.json",
    root / "ceo_memory" / "crm_leads.json",
    root / "ceo_memory" / "crm_activities.json",
]
...
try:
    ...
    if not customers:
        errors.append("No customer created")

    if not leads:
        errors.append("No lead created")

    if len(activities) < 2:
        errors.append("CRM activities were not recorded")

except Exception as exc:
    errors.append(f"CRM memory error: {exc}")

print("--------------------------------------------")
print("Phase 16 Step 1 verification")
...
echo
echo "============================================================"
echo " PHASE 16 STEP 1 INSTALLED"
echo " Errors: 0"
echo " Warnings: 0"
echo "============================================================"
echo
echo "Commands:"
echo "  python companyos/crmctl add-customer \"NAME\" \"PHONE\" \"EMAIL\""
echo "  python companyos/crmctl add-lead \"CUSTOMER\" \"PROJECT\" AMOUNT SOURCE"
echo "  python companyos/crmctl customers"
echo "  python companyos/crmctl leads"
echo "  python companyos/crmctl activities"
echo "  python companyos/crmctl status"
