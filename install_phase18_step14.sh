...
LOGS="$ROOT/logs"
BACKUP="$ROOT/backups/phase18_step14_$(date +%Y%m%d_%H%M%S)"

cd "$ROOT"
mkdir -p "$AGENTS" "$CTL" "$MEMORY" "$LOGS" "$BACKUP"

echo "============================================================"
echo " Phase 18 Step 14 - Autonomous Health Supervisor"
echo "============================================================"

for file in \
  "$AGENTS/autonomous_health_supervisor.py" \
  "$CTL/healthctl" \
  "$MEMORY/autonomous_health_config.json" \
  "$MEMORY/autonomous_health_state.json" \
  "$MEMORY/autonomous_health_report.json" \
  "$MEMORY/autonomous_health_audit.json" \
...
cat > "$MEMORY/autonomous_health_config.json" <<'JSON'
{
  "enabled": true,
  "automatic_health_checks": true,
  "automatic_recovery_trigger": true,
  "automatic_scheduler_restart": true,
  "automatic_watchdog_restart": true,
  "automatic_orchestrator_recovery": true,
  "maximum_recovery_attempts_per_cycle": 3,
...
cat > "$AGENTS/autonomous_health_supervisor.py" <<'PY'
#!/usr/bin/env python3
...
chmod +x "$AGENTS/autonomous_health_supervisor.py"

cat > "$CTL/healthctl" <<'PY'
#!/usr/bin/env python3
import subprocess
import sys
from pathlib import Path
...
root = Path.home() / "companyos"
agent = root / "agents" / "autonomous_health_supervisor.py"
...
echo "[2/6] Running health supervision..."
python "$CTL/healthctl" run || true

echo "[3/6] Adding health supervisor to scheduler..."
python - <<'PY'
import json
from pathlib import Path
...
job = {
    "id": "autonomous-health-supervisor",
    "enabled": True,
...
echo "[5/6] Checking health supervisor..."
python "$CTL/healthctl" status
...
required = [
    root / "agents" / "autonomous_health_supervisor.py",
    root / "companyos" / "healthctl",
    root / "ceo_memory" / "autonomous_health_config.json",
    root / "ceo_memory" / "autonomous_health_state.json",
    root / "ceo_memory" / "autonomous_health_report.json",
...
try:
    ...
    if int(state.get("cycle_count", 0)) < 1:
        errors.append("No health supervision cycle completed")

    scheduler = json.loads(required[5].read_text(encoding="utf-8"))
    job = next(
        (
            x for x in scheduler.get("jobs", [])
            if x.get("id") == "autonomous-health-supervisor"
        ),
        None,
    )

    if not job:
        errors.append("Health supervisor scheduler job missing")
    elif job.get("enabled") is not True:
        errors.append("Health supervisor scheduler job disabled")

except Exception as exc:
    errors.append(f"Verification data error: {exc}")

print("--------------------------------------------")
print("Phase 18 Step 14 verification")
...
echo
echo "============================================================"
echo " PHASE 18 STEP 14 INSTALLED"
echo " AUTONOMOUS HEALTH SUPERVISOR ACTIVE"
echo " Errors: 0"
echo " Warnings: 0"
echo "============================================================"
echo
echo "Commands:"
echo "  python companyos/healthctl run"
echo "  python companyos/healthctl status"
echo
echo "Autonomous schedule:"
echo "  Health supervision runs every 10 minutes"
