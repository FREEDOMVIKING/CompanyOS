...
echo "$(date -Iseconds) resource_aware_worker_start interval=$INTERVAL mode=${COMPANYOS_AUTONOMOUS_FINANCIAL_MODE:-dry_run}" >> "$LOGFILE"

while true; do
  python tools/companyos_resource_aware_queue_once.py >> "$LOGFILE" 2>&1
  sleep "$INTERVAL"
done
