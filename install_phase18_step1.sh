...
BACKUP="$ROOT/backups/phase18_step1_$(date +%Y%m%d_%H%M%S)"

cd "$ROOT"
mkdir -p "$TESTS" "$MEMORY" "$BACKUP"

echo "============================================================"
echo " Phase 18 Step 1 - End-to-End Business Simulation Test"
echo "============================================================"

for file in \
  "$TESTS/end_to_end_business_test.py" \
  "$MEMORY/end_to_end_test_report.json"
do
  if [ -f "$file" ]; then
    cp -a "$file" "$BACKUP/"
  fi
...
cat > "$TESTS/end_to_end_business_test.py" <<'PY'
#!/usr/bin/env python3
...
chmod +x "$TESTS/end_to_end_business_test.py"

echo "[1/4] Compiling..."
python -m py_compile "$TESTS/end_to_end_business_test.py"

echo "[2/4] Running end-to-end simulation..."
python "$TESTS/end_to_end_business_test.py"
...
root = Path.home() / "companyos"
report_path = root / "ceo_memory" / "end_to_end_test_report.json"

errors = []

if not report_path.exists():
    errors.append("End-to-end report was not created")
else:
    ...
    if summary.get("errors") != 0:
        errors.append(
            f"End-to-end report contains {summary.get('errors')} error(s)"
        )

    safety = data.get("safety", {})
    for field, value in safety.items():
        if value is not False:
            errors.append(f"Safety field {field} is not false")

print("--------------------------------------------")
print("Phase 18 Step 1 verification")
print(f"Errors: {len(errors)}")
print("Warnings: 0")
...
echo
echo "============================================================"
echo " PHASE 18 STEP 1 TEST PASSED"
echo " Errors: 0"
echo " Warnings: 0"
echo "============================================================"
echo
echo "Report:"
echo "  ~/companyos/ceo_memory/end_to_end_test_report.json"
echo
echo "Run again:"
echo "  python tests/end_to_end_business_test.py"
