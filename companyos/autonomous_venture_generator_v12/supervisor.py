...
RUN=HOME/".companyos_enterprise_v12"
...
def main():
    ...
    while not STOP.exists():
        if child is None or child.poll() is not None:
            ...
            log=open(RUN/"v12.log","ab",buffering=0)
            child=subprocess.Popen([sys.executable,"-u","-m","companyos.autonomous_venture_generator_v12.server"],
                                   cwd=str(HOME),stdout=log,stderr=subprocess.STDOUT,
                                   stdin=subprocess.DEVNULL,start_new_session=True)
            PID.write_text(str(child.pid)); write_state("running",child.pid,restarts)
        time.sleep(5)
