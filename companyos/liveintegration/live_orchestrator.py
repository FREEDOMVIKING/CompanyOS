...
class FinalLiveExecutionOrchestrator:
    def __init__(self, root, allowlist=None):
        ...
        # Explicit caller-supplied allowlist always wins.
        # Otherwise load the persistent default-deny financial allowlist.
        if allowlist is not None:
            self.allowlist = set(allowlist)
        else:
            allowlist_path = self.root / "companyos_runtime" / "financial_allowlist.json"
            try:
                data = json.loads(allowlist_path.read_text(encoding="utf-8"))
                self.allowlist = set(data.get("addresses", []))
            except (FileNotFoundError, json.JSONDecodeError, OSError, TypeError):
                # Fail closed: unavailable/corrupt state becomes an empty allowlist.
                self.allowlist = set()
    ...
    def execute_once(
        ...
        *,
        ...
        # Phase 64 V2: consume the one-shot authorization BEFORE crossing
        ...
        receipt=self.receipts.append({
            ...
            "pending_recovery":result.get("pending_recovery"),
            "execution_result":result
        })

        # Lock only after a real execution-gate attempt.
        if execution_gate_reached:
            lock=self.postlock.engage("controlled_live_attempt_complete")
        else:
            lock={
                "success":True,
                "status":"post_execution_lock_not_required",
                "state":{"locked":False}
            }
