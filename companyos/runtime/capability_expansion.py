...
def cycle():
 ...
 if gap["id"] in inv:
  if not _capability_allowed(gap["id"]):
   st.update(status="existing_capability_quarantined",healthy=True)
 ...
 if gen.get("ok"):
  module_content,_,_=_classify_generated_contents(gen.get("plan") or {},gap["id"])
  if module_content:
   is_dup,dup_id=_is_duplicate_generated_source(module_content)
   if is_dup:
    st.update(status="candidate_rejected_duplicate",healthy=True,duplicate_of=dup_id)
    atomic(STATE,st);return st
 if not gen.get("ok"):
...
# COMPANYOS_CAPABILITY_FEEDBACK_INTEGRATION_V7
def _capability_feedback():
 return load(RT/"capability_feedback_state.json",{})

def _capability_allowed(capability_id):
 rec=(_capability_feedback().get("capabilities") or {}).get(capability_id,{})
 return rec.get("status")!="quarantined"

def _candidate_ast_fingerprint(source):
 try:
...
def _is_duplicate_generated_source(source):
 fp=_candidate_ast_fingerprint(source)
 for cid,rec in (_capability_feedback().get("capabilities") or {}).items():
  if rec.get("fingerprint")==fp and rec.get("status")!="quarantined":
   return True,cid
 return False,None
