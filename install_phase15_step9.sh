...
BACKUP="$ROOT/backups/phase15_step9_$(date +%Y%m%d_%H%M%S)"

cd "$ROOT"
mkdir -p "$AGENTS" "$CTL" "$MEMORY" "$RELEASES" "$BACKUP"

echo "============================================================"
echo " Phase 15 Step 9 - Release Approval and Local Activation"
echo "============================================================"
...
if [ ! -f "$MEMORY/release_approval_audit.json" ]; then
  echo '[]' > "$MEMORY/release_approval_audit.json"
fi

if [ ! -f "$MEMORY/release_activation_state.json" ]; then
...
def update_stats(store: dict[str, Any]) -> None:
    ...
    store["statistics"] = {
        ...
        "ready": sum(
            ...
            if item.get("status") in {"ready", "packaged", "approved", "active"}
        ),
        "blocked": sum(
...
def approve_latest(reason: str | None = None) -> dict[str, Any]:
    ...
    if not config.get("enabled", True):
        result = {"success": False, "status": "release_approval_disabled"}
...
def activate_latest() -> dict[str, Any]:
    ...
    if release.get("status") not in {"approved", "active"}:
        result = {
            ...
            "next_command": "python companyos/releaseapprovalctl approve-latest",
        }
...
echo "[2/5] Running blocked activation test..."
set +e
BLOCKED_OUTPUT="$(
...
print("--------------------------------------------")
print("Phase 15 Step 9 verification")
...
echo
echo "============================================================"
echo " PHASE 15 STEP 9 INSTALLED"
echo " Errors: 0"
echo " Warnings: 0"
echo "============================================================"
echo
echo "Do not activate yet."
echo
echo "Commands:"
