...
BACKUP="$ROOT/backups/phase15_step2_$(date +%Y%m%d_%H%M%S)"

echo "============================================================"
echo " Phase 15 Step 2 - Opportunity Scoring Engine"
echo "============================================================"
...
for file in \
    ...
    "$COMPANYOS/scorectl" \
    "$MEMORY/opportunity_scoring_config.json" \
    "$MEMORY/opportunity_rankings.json" \
    "$MEMORY/opportunity_scoring_health.json"
do
    if [ -f "$file" ]; then
        cp -a "$file" "$BACKUP/"
    fi
...
echo "[2/9] Creating scoring configuration..."

cat > "$MEMORY/opportunity_scoring_config.json" <<'JSON'
{
  "enabled": true,
  "automatic_scoring": false,
  "automatic_selection": false,
  "automatic_project_creation": false,
  "minimum_qualified_score": 60,
  "minimum_recommended_score": 72,
  "maximum_recommended_opportunities": 5,
  "weights": {
    "market_demand": 0.18,
...
echo "[3/9] Creating scoring memory..."

cat > "$MEMORY/opportunity_rankings.json" <<'JSON'
{
  "schema_version": 1,
  "rankings": [],
...
echo "[4/9] Creating Opportunity Scoring agent..."

cat > "$AGENTS/opportunity_scorer.py" <<'PY'
#!/usr/bin/env python3
...
chmod +x "$AGENTS/opportunity_scorer.py"

echo "[5/9] Creating scoring control command..."

cat > "$COMPANYOS/scorectl" <<'PY'
#!/usr/bin/env python3
...
ROOT = Path.home() / "companyos"
SCORER = ROOT / "agents" / "opportunity_scorer.py"


def main() -> int:
    if not SCORER.exists():
        print("Opportunity Scoring Engine is not installed.")
        return 1

    return subprocess.call(
        [sys.executable, str(SCORER), *sys.argv[1:]],
        cwd=ROOT,
    )
...
chmod +x "$COMPANYOS/scorectl"

echo "[6/9] Registering scoring capability..."
...
capability = {
    "id": "opportunity_scoring_engine",
    "name": "Opportunity Scoring Engine",
    "module": "agents.opportunity_scorer",
    "enabled": True,
    "automatic_selection": False,
    "automatic_project_creation": False,
    "owner_approval_required": True,
...
capabilities[:] = [
    ...
    if not (
        ...
        and item.get("id") == "opportunity_scoring_engine"
    )
]
...
print("Opportunity Scoring Engine registered.")
PY

echo "[7/9] Compiling scoring engine..."

python -m py_compile \
    "$AGENTS/opportunity_scorer.py" \
    "$COMPANYOS/scorectl"

echo "Opportunity Scoring Engine compilation passed."

echo "[8/9] Running scoring tests..."

python "$COMPANYOS/scorectl" score
python "$COMPANYOS/scorectl" status
python "$COMPANYOS/scorectl" list 5
python "$COMPANYOS/scorectl" best

echo "[9/9] Running Phase 15 verification..."
...
required = [
    root / "agents" / "opportunity_scorer.py",
    root / "companyos" / "scorectl",
    root / "ceo_memory" / "opportunity_scoring_config.json",
    root / "ceo_memory" / "opportunity_rankings.json",
...
try:
    ...
    if not isinstance(rankings.get("rankings"), list):
        errors.append("Rankings memory has invalid structure")

    if not rankings.get("rankings"):
        errors.append("No opportunities were scored")

    if not rankings.get("best_opportunity"):
        errors.append("Best opportunity was not selected")

except Exception as exc:
    errors.append(f"Rankings memory error: {exc}")
...
try:
    ...
    if not opportunities.get("opportunities"):
        errors.append("No discovered opportunities available")

except Exception as exc:
    errors.append(f"Opportunity memory error: {exc}")

print("--------------------------------------------")
print("Phase 15 Step 2 verification")
...
echo
echo "============================================================"
echo " PHASE 15 STEP 2 INSTALLED"
echo "============================================================"
echo
echo "Opportunity Scoring Engine:"
echo "  profitability scoring: enabled"
echo "  market demand scoring: enabled"
echo "  build effort scoring: enabled"
echo "  competition scoring: enabled"
echo "  launch speed scoring: enabled"
echo "  recurring revenue scoring: enabled"
echo "  strategic fit scoring: enabled"
echo "  risk scoring: enabled"
echo
echo "Automatic selection: disabled"
...
echo
echo "Commands:"
echo "  python companyos/scorectl score"
echo "  python companyos/scorectl status"
echo "  python companyos/scorectl list"
echo "  python companyos/scorectl best"
echo "  python companyos/scorectl show OPPORTUNITY_ID"
echo
echo "Memory:"
echo "  ceo_memory/opportunity_rankings.json"
echo "  ceo_memory/opportunity_scoring_health.json"
echo "  ceo_memory/opportunity_scoring_audit.json"
echo
