...
VENTURES_DIR="$ROOT_DIR/company_ventures"
MANIFEST_FILE="$ROOT_DIR/companyos/manifest.json"

echo "============================================================"
echo " Installing Business Incubator Module"
echo "============================================================"

mkdir -p \
    "$AGENTS_DIR" \
    "$MEMORY_DIR/incubator_reports" \
    "$VENTURES_DIR"

touch "$AGENTS_DIR/__init__.py"

echo
echo "[1/6] Creating Incubator Agent..."

cat > "$AGENTS_DIR/incubator_agent.py" <<'PYTHON'
#!/usr/bin/env python3
...
def incubate_idea(task: dict[str, Any]) -> dict[str, Any]:
    ...
    generated_tasks = [
        create_task(
            ...
            action="research_business_opportunity",
            project_id=project_id,
            payload={
                "venture_id": venture_id,
                "idea_id": idea_id,
...
chmod +x "$AGENTS_DIR/incubator_agent.py"

echo
echo "[2/6] Initializing incubator memory..."

[ -f "$MEMORY_DIR/ventures.json" ] || \
    printf '[]\n' > "$MEMORY_DIR/ventures.json"

[ -f "$MEMORY_DIR/incubator_state.json" ] || \
    printf '{}\n' > "$MEMORY_DIR/incubator_state.json"

[ -f "$MEMORY_DIR/incubator_summary.json" ] || \
    printf '{}\n' > "$MEMORY_DIR/incubator_summary.json"

echo
echo "[3/6] Creating incubator command utility..."

cat > "$ROOT_DIR/companyos/incubatorctl" <<'PYTHON'
#!/usr/bin/env python3
...
chmod +x "$ROOT_DIR/companyos/incubatorctl"
...
incubator = modules.setdefault("incubator", {})

incubator.update({
    "installed": True,
    "enabled": True,
    "version": "1.0.0",
    "agent": "agents/incubator_agent.py",
    "workspace": "company_ventures",
    "commands": [
        "list",
        "incubate",
        "incubate-top"
...
print("CompanyOS manifest updated.")
PYTHON

echo
echo "[5/6] Compiling and validating..."

python -m py_compile \
    "$AGENTS_DIR/incubator_agent.py" \
    "$ROOT_DIR/companyos/incubatorctl"

python "$ROOT_DIR/companyos/incubatorctl" list
...
echo
echo "============================================================"
echo " BUSINESS INCUBATOR INSTALLED SUCCESSFULLY"
echo "============================================================"
echo
echo "View current ventures:"
echo
echo "  python companyos/incubatorctl list"
echo
echo "Create a venture from the top-ranked opportunity:"
echo
echo "  python companyos/incubatorctl incubate-top"
echo
echo "Create a venture from a specific idea:"
echo
echo "  python companyos/incubatorctl incubate idea-1"
