...
BACKUP="$ROOT/backups/phase16_step3_$(date +%Y%m%d_%H%M%S)"

cd "$ROOT"
mkdir -p "$AGENTS" "$CTL" "$MEMORY" "$BACKUP"

echo "============================================================"
echo " Phase 16 Step 3 - Sales Pipeline and Follow-Up Engine"
echo "============================================================"

for file in \
  "$AGENTS/sales_pipeline_manager.py" \
  "$CTL/salesctl" \
  "$MEMORY/sales_pipeline_config.json" \
  "$MEMORY/sales_pipeline_metrics.json" \
  "$MEMORY/sales_followups.json" \
  "$MEMORY/sales_pipeline_health.json"
do
  if [ -f "$file" ]; then
    cp -a "$file" "$BACKUP/"
  fi
...
cat > "$MEMORY/sales_pipeline_config.json" <<'JSON'
{
  "enabled": true,
  ...
  "owner_approval_required_for_external_contact": true,
...
if [ ! -f "$MEMORY/sales_followups.json" ]; then
cat > "$MEMORY/sales_followups.json" <<'JSON'
{
  "schema_version": 1,
  "followups": [],
  "statistics": {
...
if [ ! -f "$MEMORY/sales_pipeline_metrics.json" ]; then
cat > "$MEMORY/sales_pipeline_metrics.json" <<'JSON'
{
  "schema_version": 1,
  "metrics": {},
  "last_updated_at": null
...
cat > "$AGENTS/sales_pipeline_manager.py" <<'PY'
#!/usr/bin/env python3
...
chmod +x "$AGENTS/sales_pipeline_manager.py"
...
root = Path.home() / "companyos"
agent = root / "agents" / "sales_pipeline_manager.py"
...
echo "[2/5] Scheduling safe internal follow-up..."
python "$CTL/salesctl" schedule-latest

echo "[3/5] Calculating sales metrics..."
python "$CTL/salesctl" metrics
python "$CTL/salesctl" pipeline
python "$CTL/salesctl" followups
python "$CTL/salesctl" status
...
required = [
    root / "agents" / "sales_pipeline_manager.py",
    root / "companyos" / "salesctl",
    root / "ceo_memory" / "sales_pipeline_config.json",
    root / "ceo_memory" / "sales_followups.json",
    root / "ceo_memory" / "sales_pipeline_metrics.json",
]
...
try:
    ...
    for field in [
        "automatic_follow_up_scheduling",
...
try:
    ...
    if not followups:
        errors.append("No follow-up was scheduled")
    ...
    errors.append(f"Sales memory error: {exc}")

print("--------------------------------------------")
print("Phase 16 Step 3 verification")
...
echo
echo "============================================================"
echo " PHASE 16 STEP 3 INSTALLED"
echo " Errors: 0"
echo " Warnings: 0"
echo "============================================================"
echo
echo "Commands:"
echo "  python companyos/salesctl schedule-latest"
echo "  python companyos/salesctl schedule LEAD_ID YYYY-MM-DD \"NOTE\""
echo "  python companyos/salesctl complete FOLLOWUP_ID \"RESULT\""
echo "  python companyos/salesctl metrics"
echo "  python companyos/salesctl pipeline"
echo "  python companyos/salesctl followups"
echo "  python companyos/salesctl status"
